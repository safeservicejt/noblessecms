...
<div class="row">
		...
		<div class="col-lg-8">
			...
			<input type="text" name="send[title]" class="form-control" placeholder="Post title..." value="<?php echo $edit['title'];?>" />
			</p>
			<p>
			<label><strong>Friendly url:</strong></label>
			<input type="text" name="send[friendly_url]" class="form-control" placeholder="Friendly url..." value="<?php echo $edit['friendly_url'];?>" />
			</p>
			<p>
			<label><strong>Content:</strong></label>
			<textarea id="editor1" class="form-control ckeditor" rows="15" name="send[content]"><?php echo $edit['content'];?></textarea>
			</p>
			<p>
			<label><strong>SEO Keywords:</strong></label>
			<input type="text" name="send[keywords]" class="form-control" placeholder="Keywords..." value="<?php echo $edit['keywords'];?>" />
			</p>

			<p>
			<label><strong>Tags: (separate by commas)</strong></label>
			<input type="text" name="tags" class="form-control" placeholder="Tags..." value="<?php echo $tags;?>" />
			</p>
...
$( document ).on( "keydown", "input.txtAuto", function() {
  ...
  if(theValue.length > 1 )
  {
    ...
    $.ajax({
     type: "POST",
     url: root_url+"usercp/news/"+keyName,
     data: ({
        do : "load",
        keyword : theValue
        }),
