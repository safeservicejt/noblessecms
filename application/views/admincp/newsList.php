		...
		<div class="row">
			<div class="col-lg-12">
				...
				<thead>
					...
					<td class="col-lg-2 hidden-xs hidden-sm">Author</td>					
					<td class="col-lg-2 hidden-xs hidden-sm">Category</td>
				...
				for($i=0;$i<$totalRow;$i++)
				{
					...
					<tr>
					<td>
					<input type="checkbox" id="cboxID" name="id[]" value="'.$posts[$i]['postid'].'" />
					</td>
					<td class="hidden-xs hidden-sm">'.$posts[$i]['firstname'].' '.$posts[$i]['lastname'].'</td>
					<td class="hidden-xs hidden-sm">'.$posts[$i]['cattitle'].'</td>
					<td><a href="'.Url::post($posts[$i]).'" target="_blank"><strong>'.stripslashes($posts[$i]['title']).'</strong></a>
					<br>
					'.$views.' '.$date_added.' '.$isFeatured.'
					</td>
					<td>'.$status.'</td>

					<td><a href="'.ROOT_URL.'admincp/news/edit/'.$posts[$i]['postid'].'" class="btn btn-xs btn-warning">Edit</a></td>
