...
$root_path=str_replace('bootstraps/ckeditor/filemanager/config','',$filePath);

$root_path=str_replace('bootstraps\ckeditor\filemanager\config','',$root_path);

define("ROOT_PATH", $root_path);

$fullUrl=$_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI'];

preg_match('/^(.*?)\/bootstraps/i', $fullUrl,$match);

$siteUrl=$match[1];

$siteHash=base64_encode($siteUrl);
...
$savePath=$root_path.'/uploads/images/'.date('Y-m-d').'/';
...
$config = array(
	...
	*/
	...
	'base_url' => ((isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] && ! in_array(strtolower($_SERVER['HTTPS']), array( 'off', 'no' ))) ? 'https' : 'http') . '://' . $siteUrl,
	...
	|
	*/
	'upload_dir' => '/uploads/images/'.date('Y-m-d').'/'.$addPath,
	...
	*/
	...
	|
	*/
	'thumbs_base_path' => 'uploads/images/'.date('Y-m-d').'/'.$addPath,
