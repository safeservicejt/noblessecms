...
class FastbbPost
{
	...
	public function get($inputData=array())
	{
		...
		$field="postid,threadid,userid,title,content,date_added,totallikes,views,status,type";
		...
		$result=array();		
		$command="select $selectFields from fastbb_post $whereQuery";
	...
	public function upViews($postid)
	{
		if((int)$postid==0)
		{
			return false;
		...
		Database::query("update fastbb_post set views=views+1 where postid='$postid'");
	}

	public function upLike($postid)
	{
		if((int)$postid==0)
		{
			return false;
		...
		Database::query("update fastbb_post set totallikes=totallikes+1 where postid='$postid'");
	}
	...
	public function update($listID,$post=array(),$whereQuery='',$addWhere='')
	{
		...
		Database::query("update fastbb_post set $setUpdates where $whereQuery $addWhere");
	...
	public function remove($post=array(),$whereQuery='',$addWhere='')
	{
		...
		// $command="select image from fastbb_post where postid in ($listID)";
		...
		$command="delete from fastbb_post where $whereQuery $addWhere";
	...
	public function insert($inputData=array())
	{
		...
		Database::query("insert into fastbb_post($insertKeys) values($insertValues)");
